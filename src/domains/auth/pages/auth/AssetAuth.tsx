...
export default function AssetAuth() {
  const navigate = useNavigate();
  const config = AUTH_MODULES.asset;
  ...
  const handleAuthSuccess = async () => {
    ...
    if (!allowed) {
      toast.error('This account does not have access to Asset Lifecycle Management. Please use a relevant role.');
      await signOut();
      return;
    }
    if (user?.email) {
      ...
      <div className="space-y-6">
        {authenticatedEmail && (
          <ModuleSandboxProvider 
            ...
            onSandboxReady={() => navigate(getModuleAwareRedirect('asset', hasRole))}
          />
        )}
        <SeedAccountsButton onSelectAccount={handleSelectAccount} module="asset" />
        <EnhancedAuthForm 
          config={config} 
