...
async function getForecasts(supabase: any, tenantId: string, filters: any = {}) {
  ...
  if (filters.geography_level) {
    query = query.eq('geography_level', filters.geography_level);
  }
  if (filters.geography_key) {
    query = query.eq('geography_key', filters.geography_key);
  }
  if (filters.product_id) {
    query = query.eq('product_id', filters.product_id);
  }
  if (filters.from_date) {
    query = query.gte('target_date', filters.from_date);
  }
  if (filters.to_date) {
    query = query.lte('target_date', filters.to_date);
  }
  if (filters.limit) {
  ...
  Object.keys(hierarchy).forEach(level => {
    Object.keys(hierarchy[level]).forEach(key => {
      ...
      group.avg_confidence = 
        group.forecasts.reduce((sum: number, f: any) => 
          sum + (Number(f.confidence_upper) + Number(f.confidence_lower)) / 2, 0
