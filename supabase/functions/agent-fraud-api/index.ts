...
async function getFraudAlerts(supabase: any, tenantId: string, filters: any = {}) {
  let query = supabase.from('fraud_alerts').select('*', { count: 'exact' }).order('created_at', { ascending: false });
...
async function calculateFraudScore(supabase: any, resourceType: string, resourceId: string, data: any): Promise<{ confidence_score: number; risk_factors: string[] }> {
  ...
  if (data?.completed_at && data?.released_at) {
    const hours = (new Date(data.completed_at).getTime() - new Date(data.released_at).getTime()) / (1000 * 60 * 60);
    if (hours < 1) {
      score += 0.2;
  ...
  return { confidence_score: Math.min(score, 1), risk_factors: riskFactors };
}
