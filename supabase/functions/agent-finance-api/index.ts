...
async function getInvoices(supabase: any, tenantId: string, filters: any = {}) {
  let query = supabase
    .from('invoices')
    .select('*, work_orders(wo_number), quotes(*)', { count: 'exact' })
...
async function getPenalties(supabase: any, tenantId: string, filters: any = {}) {
  let query = supabase
    ...
    .select('*, work_orders(wo_number, technician_id)', { count: 'exact' })
    .order('created_at', { ascending: false });

  if (filters.disputed !== undefined) {
    query = query.eq('disputed', filters.disputed);
  }
...
async function getBillingSummary(supabase: any, tenantId: string, filters: any = {}) {
  ...
  const { data: invoices } = await supabase
    .from('invoices')
    .select('status, total_amount, penalties')
